...
public class EMParameterFinder
{
	...
	{
		Node classChild = classBodyDecl.jjtGetFirstChild();
		if (classChild instanceof ASTMethodDeclaration) {
         int childNo = ((ASTMethodDeclaration)classChild).skipAnnotationsAndTypeParameters();
			ASTMethodDeclarator decl = (ASTMethodDeclarator) classChild.jjtGetChild(childNo+1);
		...
		if (classChild instanceof ASTConstructorDeclaration) {
			ASTConstructorDeclaration decl = (ASTConstructorDeclaration) classChild;
			if (methodSummary.isConstructor()) {
            int childNo = decl.skipAnnotations();
				ASTFormalParameters params = (ASTFormalParameters) decl.jjtGetChild(childNo);
				return isParametersSame(params, methodSummary);
			}
			return false;
		}

		if (classChild instanceof ASTInitializer) {
