...
public class JavaDocableImpl implements JavaDocable {
   ...
   public void printJavaDocComponents(PrintData printData, String order) {
      ...
      boolean firstLineOnCommentStart = printData.isFirstLineOnCommentStart();
      if (!onSingleLine && !firstLineOnCommentStart) {
         printData.newline();
      }
    ...
    *
    * @param  printData  the print data
    * @return            true if it can fit (and is allowed to fit) on a single line
    */
    ...
    * @param  singleLine  Description of Parameter
    */
      ...
      for (int ndx = 0; ndx < docs.size(); ndx++) {
         ...
         if (current.getType().equals("")) {
            ...
            if (JavadocTokenizer.hasContent(descr)) {
               printCurrentTag(current, printData, docs.size() == 1, singleLine);
            }
            current.setPrinted(true);
         }
      }
